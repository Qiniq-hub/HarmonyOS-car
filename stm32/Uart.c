...
static void UART_Task(void)
{
    ...
    GpioInit();
    // 配置GPIO0复用为UART1_TXD
    IoSetFunc(WIFI_IOT_GPIO_IDX_0, WIFI_IOT_IO_FUNC_GPIO_0_UART1_TXD);
    // 配置GPIO1复用为UART1_RXD
    IoSetFunc(WIFI_IOT_GPIO_IDX_1, WIFI_IOT_IO_FUNC_GPIO_1_UART1_RXD);
    ...
    while (1)
    {
        ...
        if (status == osOK)
        {
            printf("Message Queue id:%d, Get msg:%s\n", msg_rx.Idx, msg_rx.Buf);
        }
        sleep(1);
    }
}
